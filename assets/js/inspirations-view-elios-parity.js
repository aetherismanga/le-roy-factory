(() => {
  ...
  // Le filtre fonctionne même si ce correctif est chargé avant les fichiers de données VIEW.
  if (Array.isArray(window.VIEW_CATALOGUE)) {
    window.VIEW_CATALOGUE = filterCatalogue(window.VIEW_CATALOGUE);
  } else {
    const descriptor = Object.getOwnPropertyDescriptor(window, 'VIEW_CATALOGUE');
  ...
  // Les vignettes VIEW suivent ELIOS : aucune disponibilité ni aucun prix sur la carte.
  const installStyle = () => {

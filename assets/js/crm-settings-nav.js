  ...
  function fixSettingsPagePosition() {
    ...
    if (!document.getElementById('lrf-settings-mobile-fix')) {
      ...
      style.textContent = `
        ...
        body.crm-body .settings-topbar{padding-right:74px!important;}
        #lrf-settings-close{position:absolute;right:18px;top:50%;transform:translateY(-50%);z-index:6;width:46px;height:46px;border-radius:15px;border:1px solid rgba(212,175,55,.65);background:rgba(255,255,255,.96);color:#171717;font-size:2rem;line-height:1;display:grid;place-items:center;cursor:pointer;box-shadow:0 8px 22px rgba(0,0,0,.15);transition:transform .16s ease,background .16s ease,border-color .16s ease;}
        #lrf-settings-close:hover,#lrf-settings-close:focus-visible{background:#fff7db;border-color:#d4af37;outline:none;transform:translateY(-50%) scale(1.04);}
        @media(max-width:900px){
          ...
          body.crm-body .settings-topbar{position:relative!important;top:0!important;margin:0!important;min-height:auto!important;padding:18px 72px 16px 16px!important;z-index:2!important;}
          ...
          #lrf-settings-close{right:14px;width:44px;height:44px;border-radius:14px;font-size:1.9rem;}
        }
      `;

...
function init(){
  document.addEventListener("click",e=>{
    const btn=e.target.closest(".btn-mail-row,.btn-edit-row");const row=btn?.closest("#clients-table-body tr");if(!btn||!row)return;
  ...
  onSnapshot(collection(db,"clients"),snap=>{clients=[];snap.forEach(d=>clients.push({id:d.id,...d.data()}));});
}

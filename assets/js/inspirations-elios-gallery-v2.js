  ...
  function buildGallery(card, id) {
    ...
    if (colorHeading && oldColorChips && Array.isArray(product.colors) && product.colors.length) {
      ...
      colorBox.innerHTML = product.colors.map((color, index) => {
        ...
        return `<button type="button" data-elios-color="${esc(color)}" data-color-index="${index}" class="${isVerified ? 'verified' : ''}" title="${isVerified ? 'Visuels HD vérifiés pour cette couleur' : 'Visuels de collection disponibles'}">${esc(color)}</button>`;
      }).join('');

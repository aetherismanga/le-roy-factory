(()=>{
  ...
  const readMemory=()=>{
    try{return JSON.parse(localStorage.getItem(MEMORY_KEY)||'{"history":[]}')||{history:[]}}
  ...
  style.textContent=`
    ...
    @media(max-width:700px){
      ...
      #lrf-web-jarvis .lrf-jw-label{grid-column:1/-1;font-size:.72rem;color:#9A7A00;padding:3px 4px 0}
      #lrf-web-jarvis input{font-size:.9rem;padding:5px 3px 8px}
      #lrf-web-jarvis button{width:36px;height:36px}
      #lrf-web-jarvis .lrf-jw-answer{font-size:.88rem}
    }
  `;
  ...
  root.innerHTML=`<div class="lrf-jw-inner">
    <form class="lrf-jw-form" autocomplete="off">
      <span class="lrf-jw-label">JARVIS IA</span>
      <input type="search" maxlength="1200" placeholder="Posez votre question métier…" aria-label="Question à Jarvis IA">
      <button type="submit" aria-label="Envoyer à Jarvis">➤</button>
    </form>
    <div class="lrf-jw-answer" role="status" aria-live="polite"></div>
  </div>`;
  ...
  const answer=root.querySelector('.lrf-jw-answer');

  const setAnswer=(text,meta='')=>{
    answer.textContent=String(text||'');
    if(meta){const small=document.createElement('span');small.className='lrf-jw-meta';small.textContent=meta;answer.appendChild(small)}
  ...
  function executeAction(a){
    ...
    if(a.type==='open_app_page'){
      const map={clients:'clients.html',agenda:'agenda.html',carte:'carte.html',statistiques:'statistiques.html','comptes-rendus':'comptes-rendus.html',mails:'mails-groupes.html',partenaires:'partenaires.html',tarifs:'tarifs-pro.html',catalogues:'catalogues.html'};
      let url=map[a.page]||'';
      if(a.page==='client'&&a.clientId)url=`clients.html?edit=${encodeURIComponent(a.clientId)}`;
      if(url&&a.partner&&(a.page==='tarifs'||a.page==='catalogues'))url+=`?jarvis=${encodeURIComponent(a.partner)}`;
      if(url)setTimeout(()=>location.href=url,700);
    }
    if(a.type==='prepare_group_mail'){
      const q=new URLSearchParams({jarvisPrepare:'1',jarvisType:a.recipientType||'client'});
      if(a.partner)q.set('jarvisPartner',a.partner);if(a.year)q.set('jarvisYear',a.year);if(a.departement)q.set('jarvisDept',String(a.departement).replace(/^FR-/i,''));if(a.documentType)q.set('jarvisDocumentType',a.documentType);
      setTimeout(()=>location.href=`mails-groupes.html?${q}`,800);
    }
  }
  ...
  form.addEventListener('submit',async e=>{
    ...
    button.disabled=true;setAnswer('Jarvis réfléchit…');
    const memory=readMemory();
    const isAgent=Boolean(localStorage.getItem('agentLoggedIn'))||document.body.classList.contains('crm-body');
    try{
      const r=await fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history:(memory.history||[]).slice(-12),page:location.pathname.split('/').pop()||'index.html',surface:isAgent?'agent_web':'public_web'})});
      const data=await r.json().catch(()=>({}));
      if(!r.ok||!data.success)throw new Error(data.error||`Serveur ${r.status}`);
      setAnswer(data.answer||'Demande traitée.',data.model?`Modèle : ${data.model.replace('gpt-5.6-','')}`:'');
      remember(text,data.answer||'Demande traitée.');
      input.value='';
      const actions=Array.isArray(data.actions)?data.actions:[];if(actions.length)executeAction(actions[0]);
    }catch(err){setAnswer(`Jarvis IA est momentanément indisponible : ${err?.message||err}`)}

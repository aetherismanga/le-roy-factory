(()=>{
  ...
  const manifest=document.createElement('link');manifest.rel='manifest';manifest.href='/manifest.webmanifest?v=20260915-brand-final3';document.head.appendChild(manifest);
  ...
  const icon=document.createElement('link');icon.rel='apple-touch-icon';icon.href='/apple-touch-icon.png?v=20260915-brand-final3';document.head.appendChild(icon);

  if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=20260915-brand-final3',{updateViaCache:'none'}).then(reg=>reg.update()).catch(console.warn));}

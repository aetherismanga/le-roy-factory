import { db, authFetch } from './firebase.js';
...
function currentClient(){
  ...
  return clients.find(c=>norm(c.societe)===norm(name))||null;
}
...
.account-send-overlay{position:fixed;inset:0;background:rgba(0,0,0,.58);z-index:100000;display:none;align-items:center;justify-content:center;padding:1rem}.account-send-box{width:min(620px,95vw);background:#fff;border-radius:14px;padding:1.35rem;box-shadow:0 20px 60px rgba(0,0,0,.3);border:1px solid rgba(212,175,55,.45)}.account-send-box h3{margin:0 0 .8rem}.account-send-grid{display:grid;grid-template-columns:1fr 180px;gap:.75rem}.account-send-box label{display:block;font-size:.78rem;font-weight:800;margin:.65rem 0 .3rem}.account-send-box input,.account-send-box select,.account-send-box textarea{width:100%;box-sizing:border-box;padding:.7rem;border:1px solid #d8d1c5;border-radius:8px;font:inherit}.account-send-box textarea{min-height:190px;resize:vertical}.account-send-actions{display:flex;gap:.65rem;justify-content:flex-end;margin-top:1rem;flex-wrap:wrap}.account-send-btn{border:1px solid #D4AF37;background:#111;color:#FFD700;border-radius:8px;padding:.7rem 1rem;font-weight:800;cursor:pointer}.account-send-btn:disabled{opacity:.55;cursor:wait}.account-send-cancel{border:1px solid #d8d1c5;background:#fff;border-radius:8px;padding:.7rem 1rem;font-weight:700;cursor:pointer}@media(max-width:620px){.account-send-grid{grid-template-columns:1fr}}
...
function fillMessage(c){
  const type=document.getElementById('account-send-type').value;const update=type==='mise_a_jour';const code=c?.codeClient||'';const dep=clientDep(c);const link=`${FORM}${update?'?type=mise_a_jour':''}`;
  document.getElementById('account-send-subject').value=update?'Mise à jour sécurisée de votre compte LE ROY FACTORY':'Ouverture de votre compte LE ROY FACTORY';
  document.getElementById('account-send-message').value=update?`Bonjour,\n\nAfin de mettre à jour votre compte professionnel LE ROY FACTORY, merci de compléter le formulaire suivant :\n\n${link}\n\nVos identifiants :\nCode client : ${code||'LRF-xxxxx'}\nDépartement : ${dep||'--'}\n\nPour protéger vos informations, un code de sécurité à 6 chiffres sera envoyé à l'adresse e-mail déjà enregistrée sur votre fiche. Vos données existantes ne seront affichées qu'après cette vérification.\n\nMerci ensuite de vérifier les informations, compléter les éléments à modifier et joindre obligatoirement votre RIB et votre Kbis.\n\nCordialement,\nLE ROY FACTORY`:`Bonjour,\n\nAfin de procéder à l'ouverture de votre compte professionnel LE ROY FACTORY, merci de compléter le formulaire suivant :\n\n${link}\n\nMerci de renseigner l'ensemble des informations demandées et de joindre obligatoirement votre RIB et votre Kbis. Les documents transmis sont stockés dans l'espace sécurisé LE ROY FACTORY.\n\nCordialement,\nLE ROY FACTORY`;
}
...
async function sendForm(){
  const email=document.getElementById('account-send-email').value.trim(),subject=document.getElementById('account-send-subject').value.trim(),text=document.getElementById('account-send-message').value.trim();
  if(!email||!subject||!text){alert('E-mail, objet et message sont obligatoires.');return}
  const btn=document.getElementById('account-send-submit');btn.disabled=true;btn.textContent='Envoi…';
  try{
    const html=`<div style="font-family:Arial,sans-serif;white-space:pre-line;line-height:1.6">${esc(text).replace(/\n/g,'<br>')}</div>`;
    const r=await authFetch(SEND,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({senderMode:'both',bccRecipients:[email],subject,htmlContent:html,attachments:[],clientIds:window.__ACCOUNT_SEND_CLIENT?.id?[window.__ACCOUNT_SEND_CLIENT.id]:[]})});
    const data=await r.json().catch(()=>({}));if(!r.ok||!data.success)throw new Error(data.error||'Échec de l’envoi');
    alert('Formulaire sécurisé envoyé au client.');closeDialog();
  }catch(e){console.error(e);alert(e.message||'Impossible d’envoyer le formulaire.')}
  finally{btn.disabled=false;btn.textContent='Envoyer au client'}
}
function injectButtons(){
  const toolbar=document.querySelector('.crm-toolbar');if(toolbar&&!document.getElementById('btn-send-account-form')){const b=document.createElement('button');b.id='btn-send-account-form';b.type='button';b.className='btn-primary-gold';b.textContent='✉ Envoyer ouverture / mise à jour';b.onclick=()=>openDialog(null);toolbar.insertBefore(b,toolbar.children[1]||null)}

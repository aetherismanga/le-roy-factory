(() => {
  ...
  function apply(){
    if(!isBio())return;
    if(recoverProducts())return;
    ensureDatalist();
    const input=$('#bio-search'),host=$('#partner-products');if(!input||!host)return;
    const q=norm(input.value);const type=norm($('#bio-filter-type')?.value||''),unit=norm($('#bio-filter-unit')?.value||''),special=$('#bio-filter-special')?.value||'';let exact=null,visible=0;
    $$('.bio-card',host).forEach(card=>{const name=norm(cardName(card));const ct=norm(card.dataset.bioType||''),units=norm(card.dataset.bioUnits||'').split(' '),sp=card.dataset.bioSpecial==='1';const okQ=!q||name.includes(q),okType=!type||ct===type,okUnit=!unit||units.includes(unit),okSpecial=!special||(special==='special'?sp:!sp);const show=okQ&&okType&&okUnit&&okSpecial;card.style.display=show?'':'none';if(show)visible++;if(q&&name===q)exact=card});
    const count=$('#partner-count');if(count)count.textContent=`${visible} produit${visible>1?'s':''}`;
    if(exact&&input.dataset.bioOpened!==q){input.dataset.bioOpened=q;setTimeout(()=>exact.click(),80)}if(!q)delete input.dataset.bioOpened;
  }
  ...
  document.addEventListener('change',e=>{if(['bio-search','bio-filter-type','bio-filter-unit','bio-filter-special'].includes(e.target?.id))schedule()},true);

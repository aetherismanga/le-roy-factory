(() => {
  ...
  const SERIES=[
    {name:'Bel Air',page:10,image:'bel-air.webp'},
    {name:'Flip',page:12,image:'flip.webp'},
    {name:'Alsacia',page:13,image:'alsacia.webp'},
    {name:'Malibú',page:15,image:'malibu.webp'},
    {name:'Manila',page:17,image:'manila.webp'},
    {name:'Bora Bora',page:19,image:'bora-bora.webp'},
    {name:'New Twist',page:20,image:'new-twist.webp'},
    {name:'Oporto',page:22,image:'oporto.webp'},
    {name:'Lyon',page:23,image:'lyon.webp'},
    {name:'Munich',page:24,image:'munich.webp'},
    {name:'Época',page:25,image:'epoca.webp'}
  ];
  const TRANSLATIONS=[
    {page:10,box:[.039,.108,.245,.064],text:'L’essence du minimalisme. Ses lignes pures et ses proportions équilibrées font de Bel Air une collection polyvalente, qui s’intègre naturellement dans tout projet de salle de bains contemporaine.'},
    {page:12,box:[.039,.115,.42,.06],text:'La manette moletée affirme la personnalité de Flip. Une collection d’inspiration technique qui associe précision, ergonomie et esthétique distinctive, pour les projets où chaque détail compte.'},
    {page:13,box:[.039,.135,.42,.06],text:'Conçue pour ceux qui recherchent simplicité et fonctionnalité sans renoncer au style. Alsacia associe une esthétique épurée à une collection complète, facile à intégrer dans tous les univers de salle de bains.'},
    {page:15,box:[.039,.138,.43,.06],text:'Équilibre, légèreté et lignes épurées. Malibú offre une silhouette contemporaine, sublimée par des finitions comme Gun Grey, pensée pour les espaces actuels et élégants.'},
    {page:17,box:[.039,.13,.43,.06],text:'Une collection conçue pour personnaliser chaque détail. La combinaison des corps et des manettes dans différentes finitions permet de créer des compositions uniques à l’identité esthétique affirmée.'},
    {page:19,box:[.039,.137,.43,.06],text:'Sa manette caractéristique de type joystick donne une personnalité unique à cette collection aux géométries affirmées, conçue pour les salles de bains recherchant un design sophistiqué et une utilisation précise.'},
    {page:20,box:[.039,.13,.43,.06],text:'Lignes droites, volumes structurés et présence architecturale : chaque pièce devient un élément central de la salle de bains. Un minimalisme contemporain plein de caractère.'},
    {page:22,box:[.038,.195,.43,.064],text:'Pensée pour réduire la consommation d’énergie dès la première utilisation. Son ouverture à froid améliore l’efficacité sans modifier le confort d’usage, en associant durabilité et fonctionnalité.'},
    {page:23,box:[.039,.137,.43,.06],text:'Une collection pratique et compétitive, conçue pour les projets résidentiels et professionnels. Design fonctionnel, ouverture à froid et fiabilité adaptée à un usage intensif.'},
    {page:25,box:[.039,.127,.43,.06],text:'Inspirée de la robinetterie traditionnelle, Época revisite le charme des formes classiques avec des finitions soigneusement modernisées. Une collection pensée pour des ambiances élégantes et intemporelles.'}
  ];
  ...
  const state={data:null,technicalTranslations:[],pdf:null,zoom:1,currentPage:1,currentPdfPage:1,views:[],rendered:new Set(),observer:null,currentObserver:null,pro:false,prices:{},selected:null,location:null,mobile:mobileQuery.matches,renderGeneration:0};
  ...
  function addTranslations(holder,view){[...TRANSLATIONS,...state.technicalTranslations].forEach(item=>{if(item.page!==view.pdfPage)return;const box=boxForView(item.box,view.side);if(!box)return;const [x,y,w,h]=box,translation=document.createElement(item.kind==='title'?'strong':'p'),baseSize=Math.max(5,Math.min(14,holder.clientWidth*(holder.dataset.side ? .013 : .0065))),fontSize=item.kind?Math.max(4,Math.min(baseSize,(w*holder.clientWidth)/(item.text.length*.54))):baseSize;translation.className=`aq-translation${item.kind?` ${item.kind}`:''}`;translation.lang='fr';translation.textContent=item.text;translation.style.cssText=`left:${x*100}%;top:${y*100}%;width:${w*100}%;min-height:${h*100}%;font-size:${fontSize}px`;holder.appendChild(translation)})}
  ...
  function unloadPage(holder){
    ...
    holder.querySelectorAll('canvas,.aq-hotspot,.aq-price-overlay,.aq-cover-mode,.aq-translation').forEach(el=>el.remove());
  }
  async function renderPage(holder){
    ...
    try{
      ...
      addTranslations(holder,view);addLayers(holder,view);addUnmatchedPrices(holder,view);state.rendered.add(viewIndex)
  ...
  function createPages(){
    ...
    state.observer=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting)renderPage(e.target);else unloadPage(e.target)}),{rootMargin:state.mobile?'650px 0px':'900px 0px'});root.querySelectorAll('.aq-page').forEach(p=>state.observer.observe(p));
  ...
  function renderSeries(){if($('series-grid').children.length)return;$('series-grid').innerHTML=SERIES.map(series=>`<button type="button" class="aq-series-card" data-page="${series.page}"><img src="assets/img/aquahome-series/${series.image}" alt="Série ${series.name}" loading="lazy"><span><strong>${series.name}</strong><small>Voir la collection</small></span><b>›</b></button>`).join('')}
  function openSeries(){renderSeries();$('series-dialog').showModal()}
  ...
  function bind(){document.querySelectorAll('[data-close]').forEach(btn=>btn.onclick=()=>$(btn.dataset.close).close());$('previous-page').onclick=()=>goPage(state.currentPage-1);$('next-page').onclick=()=>goPage(state.currentPage+1);$('page-input').onchange=e=>goPage(e.target.value);$('search-button').onclick=search;$('series-button').onclick=openSeries;$('series-grid').onclick=e=>{const card=e.target.closest('[data-page]');if(!card)return;$('series-dialog').close();goPage(viewForSeries(Number(card.dataset.page)))};$('product-search').onkeydown=e=>{if(e.key==='Enter')search()};$('zoom-in').onclick=()=>setZoom(state.zoom+.15);$('zoom-out').onclick=()=>setZoom(state.zoom-.15);$('variant-select').onchange=e=>selectVariant(e.target.value);$('add-to-cart').onclick=addToCart;$('dealer-open').onclick=openDealer;$('location-input').oninput=e=>geoSearch(e.target.value);$('location-results').onclick=e=>{const btn=e.target.closest('button');if(!btn)return;const choice=e.currentTarget._choices[Number(btn.dataset.i)];state.location=choice;$('location-input').value=`${choice.city} — ${choice.postalCode}`;$('dealer-form').elements.ville.value=choice.city;$('dealer-form').elements.codePostal.value=choice.postalCode;e.currentTarget.hidden=true};$('dealer-form').onsubmit=submitDealer;window.addEventListener('lrf-pro-session-changed',refreshAccess)}
  async function init(){try{bind();cartCount();const [data,translations,pdf]=await Promise.all([fetch(DATA_URL,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('Index produits indisponible.');return r.json()}),fetch(TRANSLATIONS_URL,{cache:'no-store'}).then(r=>r.ok?r.json():{overlays:[]}),pdfjsLib.getDocument({url:PDF_URL}).promise]);state.data=data;state.technicalTranslations=translations.overlays||[];state.pdf=pdf;createPages();await refreshAccess();$('loading').hidden=true;const ref=new URLSearchParams(location.search).get('ref');if(ref){const row=findProduct(ref);if(row){$('product-search').value=row.reference;setTimeout(()=>{goPage(viewForProduct(row));openProduct(row)},300)}}}catch(error){$('loading').innerHTML=`<strong>Le catalogue interactif est momentanément indisponible.</strong><span>${error.message}</span>`;console.error(error)}}

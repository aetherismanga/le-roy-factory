(() => {
  ...
  const DATA={
    'acropoli':{
      ...
      corner:{label:'Angles / Spigoli',package:'1,00 ml',weight:'14,50 kg/ml',boxWeight:'14,50 kg',format:'Mélange / formats mixtes',thickness:'Non indiquée sur la fiche officielle',pallet:'40 cartons · 40 ml · 580 kg'}
    }
  };
  ...
  function enhance(modal){if(!modal?.classList.contains('open'))return;const data=DATA[norm(productName(modal))];if(!data)return;const body=$('.bio-body',modal);if(!body)return;let tech=$('.bio-tech',body);if(!tech){tech=document.createElement('section');tech.className='bio-tech';const desc=$('.bio-product-desc',body);(desc||body.firstChild)?.insertAdjacentElement?.('afterend',tech);if(!tech.parentNode)body.insertBefore(tech,body.firstChild)}tech.innerHTML=`<h3>Caractéristiques produit</h3><p class="bio-tech-desc">${esc(data.desc)}</p><div class="bio-color-list">${data.colors.map(c=>`<span class="bio-color-chip">${esc(c)}</span>`).join('')}</div><div class="bio-tech-grid">${cardHtml(data.flat)}${cardHtml(data.corner)}</div>`;rewriteGallery(modal,data)}
  function rewriteGallery(modal,data){const name=productName(modal);$$('.bio-gallery-label',modal).forEach(label=>{const main=label.closest('.bio-gallery-main');const img=$('img',main);const src=(img?.src||'')+' '+(img?.alt||'')+' '+(label.textContent||'');const hit=data.colors.find(c=>norm(src).includes(norm(c)));label.textContent=hit?`${name} · couleur ${hit}`:`${name} · visuel d’ambiance`});$$('.bio-gallery-thumb img',modal).forEach(img=>{const src=(img.src||'')+' '+(img.alt||'');const hit=data.colors.find(c=>norm(src).includes(norm(c)));img.alt=hit?`${name} · couleur ${hit}`:`${name} · visuel d’ambiance`})}

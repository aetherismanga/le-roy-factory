(() => {
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='bio-final-ui-style';s.textContent=`
      .bio-colors{display:none!important}
      .bio-head{position:relative!important;padding-right:94px!important;min-height:118px!important;box-sizing:border-box!important}
  ...
  function colors(modal){
    ...
    const signature=key+'|'+entry.colors.join('|');
    if(sec.dataset.signature===signature)return;
    sec.dataset.signature=signature;
    const staticMap=MAP[key]||{};
    sec.innerHTML=`<h3>Toutes les couleurs disponibles</h3><p>Chaque couleur est présentée avec son visuel officiel Biopietra lorsqu’il est publié sur la fiche fabricant.</p><div class="bio-final-grid">${entry.colors.map(c=>{const u=staticMap[c]||'';return `<button type="button" class="bio-final-color" data-color="${esc(c)}">${u?`<img src="${esc(u)}" alt="${esc(name+' · '+c)}" loading="lazy">`:`<img data-dynamic="1" alt="${esc(name+' · '+c)}" loading="lazy">`}<strong>${esc(c)}</strong></button>`}).join('')}</div><div class="bio-final-selected"></div>`;
    $$('.bio-final-color',sec).forEach(card=>{const color=card.dataset.color;card.onclick=()=>setColor(modal,color,card);const img=$('img[data-dynamic]',card);if(img&&window.BIOPIETRA_PRODUCT_OPTIONS?.imageFor){window.BIOPIETRA_PRODUCT_OPTIONS.imageFor(name,color).then(x=>{if(x?.url&&img.isConnected){img.src=x.url;img.removeAttribute('data-dynamic')}}).catch(()=>{})}});
  }
  function enhance(modal){if(!modal?.classList.contains('open'))return;style();close(modal);colors(modal)}
  let scanTimer=0;function scan(){clearTimeout(scanTimer);scanTimer=setTimeout(()=>$$('.bio-modal.open').forEach(enhance),30)}
  document.addEventListener('click',scan,true);
  new MutationObserver(ms=>{if(ms.some(m=>m.type==='attributes'||[...m.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('.bio-modal,.bio-head,.bio-body')||n.querySelector?.('.bio-modal,.bio-head,.bio-body')))))scan()}).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});
  window.addEventListener('lrf-biopietra-rerendered',scan);

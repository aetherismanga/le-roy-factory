(() => {
  ...
  const CONSUMABLES={
    'ecokoll 25 kg':{q:'Ecokoll',title:'Ecokoll',text:'Colle respirante recommandée pour la pose des parements Biopietra. Elle participe au système de pose complet de la marque et favorise une adhérence durable sur supports correctement préparés.'},
    'multikoll 25 kg':{q:'Multikoll',title:'Multikoll',text:'Adhésif respirant Biopietra destiné à la pose par double encollage. Il est prévu pour assurer une prise régulière des éléments de parement et existe en variantes adaptées à certaines teintes.'},
    'biostucco 25 kg':{q:'Biostucco',title:'Biostucco',text:'Mortier-joint respirant conçu pour la finition des parements Biopietra. Disponible en plusieurs coloris, il permet d’accorder le joint à la pierre et de renforcer le rendu naturel du mur.'},
    'biofin':{q:'Biofin',title:'Biofin',text:'Traitement de protection oléo-hydrofuge pour les parements Biopietra. Il aide à protéger les surfaces de l’humidité, des salissures et des agents extérieurs sans modifier l’aspect de la pierre.'}
  };
  ...
  function style(){
    ...
    s.textContent=`
      ...
      .bio-section-products .bio-card-top{align-items:stretch}.bio-section-products .bio-card-top img{width:150px!important;height:145px!important;object-fit:cover!important;border-radius:14px!important;background:#eee8de!important}.bio-section-products .bio-card-top{min-height:170px}.bio-section-products .bio-card-top>div{align-self:center}.bio-section-products .bio-card[data-bio-product-image="loading"] .bio-card-top img{opacity:.55}.bio-section-products .bio-card[data-bio-product-image="ready"] .bio-card-top img{opacity:1;transition:opacity .2s ease}
      ...
      @media(max-width:760px){.bio-about{grid-template-columns:1fr;margin:12px 0}.bio-about-media,.bio-about-media img{min-height:210px;max-height:250px}.bio-about-copy{padding:18px}.bio-about p{font-size:.87rem}.bio-section-head{align-items:flex-start;flex-direction:column;gap:3px}.bio-section-grid{grid-template-columns:1fr}.bio-section-products .bio-card-top img{width:145px!important;height:145px!important}.bio-section-products .bio-card-top{min-height:165px}}
    `;
  ...
  async function officialImage(search){
    ...
    const promise=(async()=>{
      try{
        const url='https://biopietra.com/wp-json/wp/v2/media?media_type=image&per_page=30&search='+encodeURIComponent(search);
  ...
  function ensureIntro(){
    ...
    if(!box){
      ...
      box.innerHTML=`<div class="bio-about-media"><img id="bio-about-img" src="https://biopietra.com/wp-content/uploads/2016/08/Acropoli-M96-Torre-870x870.jpg" alt="Réalisation officielle Biopietra" loading="lazy"></div><div class="bio-about-copy"><div class="bio-about-kicker">Biopietra, c’est quoi ?</div><h3>La pierre régénérée italienne pensée pour construire durablement.</h3><p>Biopietra conçoit des parements écologiques en pierre naturelle régénérée pour l’intérieur comme l’extérieur. La marque associe l’esthétique minérale de la pierre à une solution plus légère, respirante et facile à mettre en œuvre, avec une fabrication 100 % Made in Italy, un marquage CE et une démarche orientée bioarchitecture. Les produits sont conçus pour durer, résister aux variations climatiques et limiter l’impact environnemental, sans COV, résines ni formaldéhyde.</p><div class="bio-about-points"><span>Intérieur & extérieur</span><span>Respirant</span><span>Faible épaisseur</span><span>100 % Made in Italy</span><span>Recyclable</span><span>Marquage CE</span></div><a class="bio-about-link" href="https://biopietra.com/fr/biopietra-pierre-regeneree-bioarchitecture/" target="_blank" rel="noopener">Découvrir la technologie Biopietra ↗</a></div>`;
  ...
  function decorateProduct(card){
    if(card.dataset.bioProductImage==='ready'||card.dataset.bioProductImage==='loading')return;
    const title=$('.bio-card-top strong',card)?.textContent?.trim()||'';
    if(!title)return;
    const img=$('.bio-card-top img',card); if(!img)return;
    card.dataset.bioProductImage='loading';
    const entry=window.BIOPIETRA_PRODUCT_OPTIONS?.entry?.(title);
    const q=entry?.q||title.replace(/\s*\+\s*mix\s+[a-z0-9]+.*$/i,'').replace(/\s+mix\s+[a-z0-9]+.*$/i,'').trim();
    officialImage(q).then(src=>{
      if(src){img.src=src;img.alt=title+' — Biopietra';card.dataset.bioProductImage='ready';}
      else card.dataset.bioProductImage='missing';
    }).catch(()=>{card.dataset.bioProductImage='missing'});
  }
  ...
  function ensureSections(){
    ...
    try{
      ...
      if(!normal){normal=document.createElement('section');normal.id='bio-products-section';normal.className='bio-section bio-section-products';normal.innerHTML='<div class="bio-section-head"><div><h3>Parements & produits Biopietra</h3><p>Sélectionnez une gamme pour voir ses couleurs, caractéristiques et tarifs PRO.</p></div></div><div class="bio-section-grid"></div>';host.appendChild(normal)}
      if(!cons){cons=document.createElement('section');cons.id='bio-consumables-section';cons.className='bio-section bio-section-consumables';cons.innerHTML='<div class="bio-section-head"><div><h3>Consommables Biopietra</h3><p>Colles, joints et protection recommandés pour le système de pose Biopietra.</p></div></div><div class="bio-section-grid"></div>';host.appendChild(cons)}
      ...
      cards.forEach(card=>{
        ...
        else {decorateProduct(card);if(card.parentNode!==ngrid)ngrid.appendChild(card)}
      });
      const opt=$('#bio-filter-type option[value="accessoire pose"]'); if(opt)opt.textContent='Consommables';
    }finally{busy=false}
  }

  function sync(){if(!isBio())return;style();ensureIntro();ensureSections()}
  ...
  new MutationObserver(muts=>{if(busy)return;if(muts.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('.bio-card,#partner-products')||n.querySelector?.('.bio-card')))))schedule()}).observe(document.documentElement,{childList:true,subtree:true});

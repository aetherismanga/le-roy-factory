(() => {
  ...
  function guess(q){const all=[];if(Array.isArray(window.ELIOS_CATALOGUE))window.ELIOS_CATALOGUE.forEach(p=>all.push({name:'Elios Ceramica',s:score(p,q)}));if(Array.isArray(window.VIEW_CATALOGUE))window.VIEW_CATALOGUE.forEach(p=>all.push({name:'View Ceramica',s:score(p,q)}));if(Array.isArray(window.NEOBATH_CATALOGUE))window.NEOBATH_CATALOGUE.forEach(p=>all.push({name:'Neobath',s:score(p,q)}));all.sort((a,b)=>b.s-a.s);if(all[0]?.s>0)return all[0].name;const qn=norm(q);return names.find(n=>qn.includes(norm(n.split(' ')[0])))||'Elios Ceramica'}
  ...
  function apply(){const partner=canonical(requested||guess(query));choose(partner);setTimeout(()=>{const input=document.getElementById('v2-search');if(query&&input){input.value=query;input.dispatchEvent(new Event('input',{bubbles:true}))}document.getElementById('partner-workspace')?.scrollIntoView({behavior:'smooth',block:'start'});if(open&&query)setTimeout(()=>{const q=compact(query);const cards=[...document.querySelectorAll('#partner-products .product-card-v2')];const exact=cards.find(c=>compact(c.querySelector('h3')?.textContent||'')===q);(exact||cards[0])?.click()},180)},80)}

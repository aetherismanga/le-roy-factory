...
(()=>{
  ...
  style.textContent=`
    ...
    /* On garde seulement le résumé principal. Les 3 cartes compteur provoquaient le tremblement. */
  ...
  function normalize(){
    ...
    if(form){form.style.overflow='visible';form.style.height='auto';form.style.maxHeight='none';}
    if(modal)modal.style.overflowY='auto';
  }

  const modal=document.getElementById('client-modal');
  if(modal){
    new MutationObserver(()=>setTimeout(normalize,30)).observe(modal,{attributes:true,attributeFilter:['style','class']});
  }

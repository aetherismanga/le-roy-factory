...
(()=>{
  ...
  async function scheduleReminder(event){
    try{if(!event?.start)return;const when=new Date(event.start);const at=new Date(when.getFullYear(),when.getMonth(),when.getDate()-1,8,0,0,0);if(at.getTime()<=Date.now())return;if(!(await requestNotificationPermission()))return;const id=reminderId(event.id);await LocalNotifications.cancel({notifications:[{id}]}).catch(()=>{});await LocalNotifications.schedule({notifications:[{id,title:'LE ROY FACTORY — demain',body:`${event.title}${event.start&&!event.allDay?' à '+new Date(event.start).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}):''}`,schedule:{at},extra:{page:'agenda',eventId:event.id}}]})}catch(e){console.warn('Rappel agenda',e)}
  }
  async function scheduleAll(calendar){for(const e of calendar.getEvents())await scheduleReminder(e)}

  function markDay(event,el){
    try{const cell=el.closest('.fc-daygrid-day');if(!cell)return;const color=event.backgroundColor||'#D4AF37';cell.style.boxShadow=`inset 0 0 0 2px ${color}`;cell.dataset.lrfHasEvent='1'}catch{}
  }

  function prepareGoogleStatus(){
    const box=document.querySelector('.google-sync-status');const btn=document.getElementById('btn-auth-google');const txt=document.getElementById('sync-text');
    if(!box||!btn||!txt)return;
    const note=document.createElement('div');note.id='lrf-google-native-note';note.textContent='Connexion Google permanente : passage en authentification Android native requis. Le bouton web intégré est désactivé dans l’application.';box.insertAdjacentElement('afterend',note);
    btn.textContent='Google Android';
    btn.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();alert('Connexion Google Calendar Android : la partie application est prête. Il reste à créer le client OAuth Android dans Google Cloud pour fr.leroyfactory.crm, puis je branche la connexion permanente.');},{capture:true});
    txt.textContent='Google Calendar : connexion Android à finaliser';
  }
  ...
  function makeSheet(calendar){
    ...
    const close=()=>wrap.classList.remove('open');wrap.querySelector('#lrf-quick-cancel').onclick=close;wrap.addEventListener('click',e=>{if(e.target===wrap)close()});
    ...
    wrap.querySelector('#lrf-quick-save').onclick=async()=>{const title=wrap.querySelector('#lrf-quick-title').value.trim();if(!title)return;const time=wrap.querySelector('#lrf-quick-time').value;const type=wrap.querySelector('#lrf-quick-type').value;const desc=wrap.querySelector('#lrf-quick-desc').value.trim();const start=new Date(selectedDate||new Date());let allDay=true,end=null;if(time){const [h,m]=time.split(':').map(Number);start.setHours(h,m,0,0);allDay=false;end=new Date(start.getTime()+60*60*1000)}else start.setHours(0,0,0,0);const ev=calendar.addEvent({id:'local_'+Date.now(),title,start,end,allDay,backgroundColor:selectedColor,borderColor:selectedColor,textColor:selectedColor==='#D4AF37'?'#111':'#fff',extendedProps:{description:desc,kind:type}});saveCalendar(calendar);await scheduleReminder(ev);close()};

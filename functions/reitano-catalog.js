...
function validSource(product){return Number.isFinite(product?.sourceHT)&&product.sourceHT>0}
function prices(){const result={};Object.values(catalogue.products).forEach(product=>{if(validSource(product))result[product.key]=Math.round(product.sourceHT*.5*100)/100});return result}
...
exports.reitanoCatalog=onRequest({region:'us-central1',timeoutSeconds:60,memory:'256MiB',secrets:['SMTP_PASSWORD_JEROME']},async(req,res)=>{
  ...
  try{
    if(action==='dealer'){
      ...
      const requested=body.product||{},product=catalogue.products[clean(requested.key,80)],publicTTC=validSource(product)?Math.round(product.sourceHT*1.2*100)/100:null;
    ...
    const items=[];
    for(const line of raw){const productKey=clean(line.key,80),product=catalogue.products[productKey],quantity=Math.floor(Number(line.quantity));if(!product||!validSource(product))return res.status(400).json({success:false,error:`Prix sur demande : ${clean(line.orderReference||line.reference,50)}.`});if(!Number.isFinite(quantity)||quantity<1||quantity>999)return res.status(400).json({success:false,error:`Quantité incorrecte pour ${product.orderReference}.`});const unitHT=Math.round(product.sourceHT*.5*100)/100;items.push({...product,quantity,unitHT,totalHT:Math.round(unitHT*quantity*100)/100})}

...
const submitAccountRequest = onRequest({secrets:['SMTP_PASSWORD_JEROME','SMTP_PASSWORD_CORYNE'],timeoutSeconds:120,memory:'512MiB'},async(req,res)=>{
  ...
  try{
    ...
    if(!clean(p.societe)||!clean(p.email)||!clean(p.codePostal))return res.status(400).json({success:false,error:'Informations obligatoires manquantes.'});
    let existingClientId='';
    if(type==='mise_a_jour'){
      const code=clean(p.codeClient,20).toUpperCase(),dep=clean(p.departementAuth,3).toUpperCase();
      if(!/^LRF-\d{5}$/.test(code)||!dep)return res.status(400).json({success:false,error:'Code client ou département invalide.'});
      const snap=await db.collection('clients').where('codeClient','==',code).limit(1).get();
      if(snap.empty)return res.status(404).json({success:false,error:'Code client inconnu.'});
      const c=snap.docs[0].data();const clientDep=String(c.departement||depFromCp(c.codePostal||c.code_postal)||'').toUpperCase();
      if(clientDep!==dep)return res.status(403).json({success:false,error:'Le département ne correspond pas à ce compte client.'});
    ...
    const rawFiles=Array.isArray(p.attachments)?p.attachments:[];const total=rawFiles.reduce((n,a)=>n+Number(a.size||0),0);if(total>12*1024*1024)return res.status(413).json({success:false,error:'Pièces jointes trop volumineuses (12 Mo maximum).'});
    const attachments=[];
...
module.exports={submitAccountRequest,getAccountRequestAttachment,sendAccountPartnerMail};
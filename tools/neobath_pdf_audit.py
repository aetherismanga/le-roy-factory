...
KEYWORDS = [
    ...
    "anta", "front", "facciata", "finitur", "finish", "colore", "color",
    "lavabo", "lavabi", "washbasin", "vasca", "top", "piano", "mensola",
    "specchio", "mirror", "led", "accessor", "porta salviette", "portasalviette",
    "pied", "gamba", "gambe", "feet", "towel", "sifone", "scarico",
]
...
def audit_pdf(label: str, path: Path) -> dict:
    ...
    for page_index, page in enumerate(doc, start=1):
        ...
        page_summaries.append({
            ...
            "price_samples": prices[:18],
            "reference_samples": refs[:22],
            "dimension_samples": dims[:16],
            "image_count": len(image_meta),
            "largest_images": sorted(image_meta, key=lambda x: x["width"] * x["height"], reverse=True)[:8],
            "text_chars": len(text),
        })
